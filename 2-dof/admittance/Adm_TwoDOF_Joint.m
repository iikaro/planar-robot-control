...
x_w = 0.6;
dx_w = 0;   %m/s
K_env = diag([1e3, 0]);	%N/m        %(Ott, 2010)
B_env = diag([10, 0]);  %N.s/m
...
ts = 0.15; %haptics reflex, visual reflex = 0.25s
wn = 4/ts;
K_d = M_d*wn^2;
B_d = 2*sqrt(K_d*M_d);
...
for i = 1 : length(t) - 1
    ...
    if rem(i,dt_c/dt) == 0
        ...
        if i == 1 && K_d ~= 0
            q_v_pure(:,i) = T_error(:,i)./K_d;
        end
        
        if i > 1 && B_d ~= 0
            q_v_pure(:,i) = (T_error(:,i) + q_v_pure(:,i - 1).*B_d/dt)./(B_d/dt + K_d);
        end
        
        if i > 2
            q_v_pure(:,i) = (T_error(:,i) + q_v_pure(:,i - 1).*(2*M_d/(dt^2) + B_d/dt) - q_v_pure(:,i - 2).*M_d/(dt^2))./(M_d/(dt^2) + B_d/dt + K_d);
        end
        
        q_v(:,i) = [0; q_v_pure(1,i) + 0*q_v_pure(2,i)];
...
%%
if (exportData)
    robotAngles = [q(1,:)', q(2,:)', zeros(size(q(1,:)'))];
    save('externalData.dat','robotAngles','-ascii')
end